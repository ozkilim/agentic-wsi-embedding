#!/usr/bin/env python3
"""
TITAN Pipeline: WSI → CONCHv1.5 patch features → TITAN slide embedding
...
Usage:
  ...
  python run_titan.py --wsi_path /path/to/slide.tif
  python run_titan.py --wsi_dir /path/to/slides/ --output_dir ./embeddings
"""
...
def process_single_wsi(wsi_path, args, titan, conch, eval_transform, device):
    """Process one WSI and return (output_path, elapsed_seconds) or (None, elapsed)."""
    wsi_name = Path(wsi_path).stem
    out_path = os.path.join(args.output_dir, f"{wsi_name}_titan_embedding.pt")
    ...
    all_feats = []
    for i in tqdm(range(0, len(tiles), args.batch_size), desc="  CONCHv1.5"):
        batch = tiles[i : i + args.batch_size]
        imgs = torch.stack([eval_transform(t) for t in batch]).to(device)
    ...
    print(f"  Patch features: {all_feats.shape}")

    feat_in = all_feats.unsqueeze(0).to(device)
    coord_in = torch.from_numpy(coords).unsqueeze(0).long().to(device)
    patch_size_lv0 = int(args.patch_size * actual_ds)

    with torch.autocast("cuda", torch.float16), torch.inference_mode():
        slide_emb = titan.encode_slide_from_patch_features(
            feat_in, coord_in, patch_size_lv0
        )
...
def main():
    parser = argparse.ArgumentParser(description="TITAN: WSI → slide embedding")
    ...
    # Load models once
    print("[Model] Loading TITAN + CONCHv1.5 ...")
    ...
    for idx, wsi_path in enumerate(wsi_paths, 1):
        ...
        try:
            out, elapsed = process_single_wsi(
                wsi_path, args, titan, conch, eval_transform, device
            )
            total_time += elapsed
